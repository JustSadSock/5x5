...
(() => {
  ...
  let single=false, difficulty='medium';
  ...
  const ms=document.getElementById('modeSelect'),
        ds=document.getElementById('difficultySelect'),
        b1p=document.getElementById('btn1p'),
        b2p=document.getElementById('btn2p'),
        rulesInit=document.getElementById('rulesBtnInitial'),
        rulesOv=document.getElementById('rulesOverlay'),
        rulesClose=document.getElementById('rulesClose'),
        board=document.getElementById('board'),
        ui=document.getElementById('ui'),
        phaseEl=document.getElementById('phase'),
        pcs=[...Array(STEPS)].map((_,i)=>document.getElementById('pc'+i)),
        acts=Array.from(document.querySelectorAll('#actions button')),
        btnDel=document.getElementById('btn-del'),
        btnNext=document.getElementById('btn-next'),
        atkOv=document.getElementById('atkOverlay');
  ...
  ds.querySelector('.easy').onclick   =()=>{ difficulty='easy';   ds.style.display='none'; startGame(); };
  ds.querySelector('.medium').onclick =()=>{ difficulty='medium'; ds.style.display='none'; startGame(); };
  ds.querySelector('.hard').onclick   =()=>{ difficulty='hard';   ds.style.display='none'; startGame(); };
  ...
  function buildBoard(){
    ...
    for(let y=0;y<5;y++)for(let x=0;x<5;x++){
      ...
      c.className='cell'; c.id=`c${x}${y}`; board.append(c);
    }
  }
  ...
  function openAttack(P){
    ...
    Object.keys(DXY).filter(d=>!usedMove[P].has(d)).forEach(d=>{
      ...
      btn.onclick=()=>{ if(tmp.includes(d)){ tmp=tmp.filter(x=>x!==d); btn.classList.remove('sel'); }
  ...
  function record(P,act){
    ...
    if(typeof act==='string'){
      ...
      if(DXY[act]){ usedMove[P].add(act);
        simPos[P].x+=DXY[act][0]; simPos[P].y+=DXY[act][1]; }
    }
    updateUI(); drawPlan(P);
  }
  ...
  function deleteLast(){
    ...
    if(typeof a==='string'){
      ...
      if(DXY[a]){
        usedMove[P].delete(a);
        simPos[P]={...units[P]};
        plans[P].forEach(r=>{ if(typeof r==='string'&&DXY[r]){
  ...
  function nextStep(){
    if(phase==='planA'&&single){
      plans.B = generatePlan(units,round,difficulty,STEPS);
      phase='execute'; btnNext.textContent='▶ Выполнить';
      clearPlan(); updateUI(); return;
    }
    if(phase!=='execute'){
  ...
  function updateUI(){
    ...
    acts.forEach(b=>{
      ...
      if(DXY[a]&&usedAtkDirs[P].has(a)){ b.disabled=true; b.classList.add('blocked'); }
    ...
    btnNext.disabled=
      single && phase==='planA'
        ? plans[P].length===0
        : plans[P].length<STEPS&&phase!=='execute';
  ...
  function drawPlan(P){
    ...
    plans[P].forEach(r=>{
      ...
      if(typeof r==='object'){
        ...
        ov.className='planMove'; ov.textContent={'up':'↑','down':'↓','left':'←','right':'→'}[r];
        cell.append(ov);
      }
    });
  }
  ...
  function clearPlan(){
    document.querySelectorAll('.planMove,.planAttack,.planShield')
      .forEach(e=>e.remove());
  }
  ...
  function execStep(){
    ...
    const aA=plans.A[step-1], aB=plans.B[step-1];
    ['A','B'].forEach(pl=>{
      const r=pl==='A'?aA:aB;
      if(typeof r==='string'&&DXY[r]){
  ...
  function render(){
    ...
    if(units.A.alive&&units.B.alive&&units.A.x===units.B.x&&units.A.y===units.B.y){
      ...
      ['A','B'].forEach(pl=>{
        ...
        p.className=pl==='A'?'playerA':'playerB';
        cell.append(p);
      });
    }
  }
