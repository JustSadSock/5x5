...
(() => {
  ...
  let units = {
    A: { x: 0, y: 2, alive: true },
    B: { x: 4, y: 2, alive: true }
  };
  ...
  function openAttack(P) {
    ...
    ok.onclick = () => {
      ...
      usedAtk[P]++;
      tmp.forEach(d => usedAtkDirs[P].add(d));
  ...
  function deleteLast() {
    ...
    if (typeof a === 'string') {
      ...
      usedAtk[P]--;
      a.dirs.forEach(d => usedAtkDirs[P].delete(d));
    }
    updateUI();
    drawPlan(P);
  }
  ...
  function execStep() {
    ...
    ['A', 'B'].forEach(pl => {
      ...
      if (r === 'shield') {
        ...
        ov.className = 'shield';
        document.getElementById(`c${u.x}${u.y}`).append(ov);
      }
    });
    render();
    ...
    if (step > STEPS) {
      ...
      usedShield = { A: 0, B: 0 };
      simPos = {
        A: { x: units.A.x, y: units.A.y },
        B: { x: units.B.x, y: units.B.y }
      };
